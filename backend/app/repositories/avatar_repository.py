...
class AvatarRepository:
    ...
    async def get_by_id(self, avatar_id: UUID) -> Avatar | None:
        result = await self.db.execute(
            select(Avatar).options(selectinload(Avatar.user), selectinload(Avatar.moderated_by)).where(
                Avatar.id == avatar_id))
    ...
    async def get_previous_avatar(self, user: User) -> Avatar:
        result = await self.db.execute(
            select(Avatar).where(Avatar.user_id == user.id,
                                 Avatar.moderation_status == AvatarModerationStatusEnum.ACCEPTED)
            .order_by(Avatar.updated_at.desc()).limit(1)
        )
        return result.scalar_one_or_none()

    async def set_avatar_status(self, avatar: Avatar, status: AvatarModerationStatusEnum, moderator: User,
    ...
    async def get_pending_list(self):
        result = await self.db.execute(select(Avatar).order_by(Avatar.updated_at).where(
            Avatar.moderation_status == AvatarModerationStatusEnum.PENDING).options(
            selectinload(Avatar.user)
        ))
